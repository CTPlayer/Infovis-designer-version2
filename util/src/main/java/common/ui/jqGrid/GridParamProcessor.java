...
public final class GridParamProcessor {
    ...
    public static final Map<String, Object> render(BaseModel baseModel, List<?> baseModels) {
        ...
        long totalCount = baseModel.getTotalCount();
        int pageSize = baseModel.getLimit() / baseModel.getPage();
        int total = (int) Math.ceil(Double.valueOf(totalCount) / pageSize);
        int page = baseModel.getPage();

        renderMap.put("page", page > total ? total : page);// 第几页
        renderMap.put("total", total);// 总页数
        renderMap.put("records", totalCount);// 总记录数
        renderMap.put("rows", baseModels);// 具体查询的数据
    ...
    public static final Map<String, Object> process(Map<?, ?> paramMap) {
        ...
        page = page > 1 ? page : 1;
        int rows = Integer.parseInt(paramMap.get("rows").toString());// 记录条数
        ...
        renderMap.put("start", (page - 1) * rows);
        renderMap.put("limit", page * rows);
        // 排序参数
        if (sidx != null) {
